...
class Core {
	...
	function httpPost($methodName_, $nvpStr_, $PayPalApiUsername, $PayPalApiPassword, $PayPalApiSignature, $PayPalMode) {
			...
			$API_UserName = urlencode($ppConf->PayPalApiUsername);
			$API_Password = urlencode($ppConf->PayPalApiPassword);
			$API_Signature = urlencode($ppConf->PayPalApiSignature);
